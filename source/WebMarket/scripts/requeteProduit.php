<?php
    ...
    */
    ...
    //Recuperation des parametres
      // Assurez-vous que la session est demarree
        ...
        if(!is_numeric($_POST['quantite_post'])) {
            ...
            if($quantite_stock < $quantite_post) {
                ...
                if($quantite_post <= 0) {
                    ...
                    // S'il existe on verifie qu'on peut rajouter
    ...
    case 1:
        if($_SESSION['admin'] != 1) {
            ...
            if(empty($_POST['titre']) 
                ...
                // Recuperation des parametres en POST
                $titre           = $_POST['titre'];
                $type            = $_POST['type'];
                $marque          = $_POST['marque']; 
                ...
                if (isset($_FILES['image_file']['name']) && !empty($_FILES['image_file']['name'])) {
                    ...
                    echo 'Image du Produit manquante.';
                    echo '<META HTTP-EQUIV="refresh" CONTENT="1;URL=?page=adminAjoutProduit">';
                    exit;
                }
                ...
                // Controle de la validite des variables stock, age, prix
                if($quantite_stock < 0 or $prix_public <= 0) {
                    ...
                    // Verifier que le titre n'existe pas dans la base de donnees
    ...
    case 2:
        // Controle des droits d'administration
        if($_SESSION['admin'] == 0) {
            echo '<META HTTP-EQUIV="refresh" CONTENT="0;URL=?page=acceuil">';
        } else {
            $results = $db->querySelect('SELECT COUNT(*) as nb_lignes FROM panier WHERE id_produit = ?', [$id_produit]);
            if($results[0]['nb_lignes'] > 0) {
                echo 'Le produit ne peut pas etre supprime, car il est dejà present dans un ou plusieurs paniers.';
            } else {
    ...
    case 3:
        ...
        if($_SESSION['admin'] == 0) {
            ...
            // Controle des parametres en POST
            if(empty($_POST['titre']) or empty($_POST['description']) or empty($_POST['marque']) 
                or empty($_POST['type']) or empty($_POST['quantite']) or empty($_POST['prix_public'])) {
                echo 'Un des champs est manquant.';
                ...
                if($quantite_stock < 0 or $prix_public < 0) {
                    ...
                    $db->execute(
                        ...
                        WHERE id_produit = ?', [$titre, $type, $marque, $image, $description, $quantite_stock, $prix_public, $id_produit]);
                    
                    echo "Produit mis à jour avec succes.";
